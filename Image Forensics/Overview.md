...
电子取证处理的大多数电子数据是图片：一个三通道的二维数组（如果是灰度图片，只有一维）。首先，如果图片是由数码相机拍摄的真实场景，最终存储的图片会因为取样传感器，处理单元的不同而天生带有不完美和可变性。并且，在到达传感器之前，`CFA(Color Filter Array)`会过滤场景的光线，让每个像素只有一种特定的颜色。传感器输出会持续插值，获取每个像素的所有三原色。这个信号会经过额外处理，比如白平衡，伽马相关性，图像增强等处理。然后才会以自定义格式存储到相机中，一般是`JPEG`格式。
...
这种方法不能用于相同型号的相机，因为它们的CFA过滤和插值算法是相同的，也不适用于压缩图片和修改过的图片（伽马相关性，平滑处理），因为这些修改会去除CFA插值的效果。
...
$$
其中$\Upsilon$是传感器的折射光强度，$\sigma$是颜色通道值，$\gamma$是伽马相关因子一般是0.45）。数乘因子$\Gamma$是零平均类噪声信号，$\Pi$是其他噪声来源的组合，包括暗电流，散粒噪声，读出噪声，$\theta_q$是量化噪声。
...
[64, 65,66]描述了如何误导两个非常重要和有用的算法：重采样探测[46]和数码相机识别算法[38].

[67]介绍了一种方法，可以揭露攻击者将一系列不同相机的图片合成一幅图误导判定的反向取证活动。

## Chapter 3 相机图片和扫描图片的区别

## Chapter 4 利用降噪技术识别源相机

## Chapter 5 快速聚类未知来源图像

很多场景下我们要做的事情是，给定`N`个图片，判定这些图片是不是`M`个不同的未知相机拍摄的，`M`一般小于等于`N`。

### 5.1 PRNU增强子

PRNU是图像信号的高频组成，可以用来在图像$I$中抽取噪声$n$，得到等式：$n=I-F(I)$。其中$F$是一个基于小波的降噪过滤器[71]，可以过滤出传感器的模式噪声。但是很多情况下，场景细节比PRNU的影响更大[72, 73]。

[72]考虑了场景细节的因素，加入了权重因子以筛选出场景细节，计算出噪声$n$。公式如下：
$$
n_e=
\begin{cases}
0 &\text{n(i, j) < -}\ \sigma\\
-cos \frac{n(i,j)\pi}{2\sigma}& \text{-}\sigma\leq n(i, j) \leq 0 \\
cos \frac{n(i,j)\pi}{2\sigma}& \text{}0\leq n(i, j) \leq \sigma \\
0&\text{n(i, j) > }\ \sigma
\end{cases}
$$
$\sigma$表示PRNU和场景细节的临界值。经过大量实验，最佳的值在$[0.05, 0.0575]$.

### 5.2 快速无监督聚类

### 5.3 实验数据

## Chapter 6 基于SIFT的复制-移动攻击检测

## Chapter 7 篡改取证

一些取证应用虽然不能非常有效的识别成像设备，但是可以提供一些暂存的定位。感觉定位信息目前不是我们关注的重点，先略过。

### 7.1 识别缺陷像素

有一种数学模型可以用来抽取暂存的定位信息。需要指出的是只有一些解决这个问题的部分线索，有一些参数还在研究。

[99]提出了一种使用贝叶斯统计自动检测缺陷的算法。但是一开始需要提供指出缺陷像素$i, j$的定位的缺陷映射。因为图像取证中很少会在有监督的环境下进行，所以一般不可能创建缺陷映射。但是[99]提出可以根据以下的规律找出候选缺陷（特别是热点）。 好像素点会衡量照到这个像素的折射光的数量。但是缺陷像素的输出可以用以下式子给出。
$$
y=I[p]+\Delta
$$
其中$I[P]$表示这个像素的折射光，$\Delta$则表示其他组件的暗补偿。为了找到缺陷像素，我们首先在图中的各个地方插值，用邻近的像素点估计本像素点的值。然后将每个像素点的估计值和实际值做对比，看是缺陷像素还是插值错误。之后根据错误数值将错误像素进行排序，选取前几个元素，因为它们有更大的错误。

另一种方法针对坏点进行识别。给出一个特定像素$(i, j)$，这个点的亮度在$t$时间点是一个单维度的信号，用$I(i, j, t)$表示。对坏点的亮度应用`DFT`,会发现无频率抖动，输出值是一定的。

识别出缺陷点以后，可以引用一个`FPN`最大相关评估来表示已知图片的具体信息。
